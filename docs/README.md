...
GitHub Pages에서는 `leemgs.mooo.com`과 `wss://leemgs.mooo.com/ssh`가 각각 SSH 호스트와 중계 서버의 기본값으로 제공됩니다. 다른 서버를 사용할 때는 입력값을 변경하면 되고, 중계 서버 주소는 브라우저에 저장되며 URL의 `?gateway=wss://...` 매개변수로 미리 지정할 수도 있습니다. 중계 서버에는 반드시 TLS, 인증/접근 제어, 연결 속도 제한과 대상 네트워크 방화벽을 적용해야 합니다.
